...
let rec desugar exprS = match exprS with
  ...
  | CompS (str, e1, e2) -> CompC (str, desugar e1, desugar e2)
...
let rec interp env r = 
  ...
  | IfC (f, th, els) -> (match (interp env f) with
                        | Bool f  -> if f = true
                                     then interp env th
                                     else interp env els
  ...
  | ArithC (str, e1, e2) -> arithEval (str, (interp env e1), (interp env e2))

  | CompC (str, e1, e2) ->  compEval (str, (interp env e1), (interp env e2))
